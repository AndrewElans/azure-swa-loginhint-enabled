...
// import { getRoles } from "./functions/getRoles.js";

const port = process.env.PORT || 3000;

const resFns = {
	fn200: function (client, urlObj) {
		client.res.setHeader("Content-Type", this.contentType ?? contentTypes.html);
		client.res.writeHead(200);
		client.res.end(
			this.jsonBody ? JSON.stringify(this.jsonBody) : (this.strBody ?? ""),
		);
	},
	fn404: (client) => {
...
const routes = {
	GET: {
		...
		"/api/login-status": (client) => {
			client.res.statusCode = 200;
			client.res.end();
		},
		"/api/login-status-pending": () => {
			// always pending endpoint
		},
		"/api/login-aad": async (client, urlObj) => {
			// e.g. api/login-aad?user=bing.whatman

			const originalUrl = "https://swa.azurestaticapps.net/.auth/login/aad";
			...
			*/
			const location = resStep2.location
				.replace("prompt=select_account", `login_hint=${user}@contoso.com`)
				.replace(
			...
			*/
			...
			const cookiesToSet = [staticWebAppsAuthContextCookieModified, nonce];
	...
	POST: {
		"/api/getroles": (client, urlObj) => {
			try {
				/* 
                    getRoles(client); 
                    in my case this function is called and forgotten
                    it logs user's login stamp and some details in the db
                */
			} catch (e) {
				console.log("/api/getroles failed", e);
			}

			resFns.fn200.call(
				{
					contentType: contentTypes.json,
					jsonBody: { roles: ["getroles_is_run"] },
				},
				client,
				urlObj,
			);
